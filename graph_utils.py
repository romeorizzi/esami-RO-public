...
def parser(path):    
    ...
    #Nodi che hanno la stessa coordinata --> aggiungo delta altrimenti non selezionabili
    x_dict = {}
    for n in dict_n:
    ...
    ###########################################################################################################
    #get rectangle centre
    ll = 0
    ru = 0
    all_c = []
    
    for key in dict_n:
        all_c.append((float(dict_n[key]['x']),float(dict_n[key]['y'])))
    ll = (10000 , 10000)
    for e in all_c:
        if e[0] <= ll[0] and e[1] <= ll[1]:
            ll = e
    ru = (0 , 0)
    for e in all_c:
        if e[0] >= ru[0] and e[1] >= ru[1]:
            ru = e    
    center = ((ll[0]+ru[0])/2,(ll[1]+ru[1])/2)
    ...
    return (graph_data,friends_e,friends_n, center)
