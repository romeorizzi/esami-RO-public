...
def generate_nb(path_yaml):
    ...
    for i in range (0,len(tasks)):
        if tasks[i]['request'] =="R0":
            ...
            request=f"__Richiesta {i} [{tasks[i]['tot_points']} punti]__: Fornire una massima sottosequenza comune tra le <b> due stringhe</b>:<br/>$s$ = {s}<br/>$t$ = {t}."
            request_lib = f"Fornire una massima sottosequenza comune tra le due stringhe s = " + str(s) + " e t = " + str(t)
            ...
            request=f"__Richiesta {i} [{tasks[i]['tot_points']} punti]__: Fornire una massima sottosequenza comune tra: <br/><b>il prefisso</b>  $t'$ = {tasks[i]['string_mod']} di $t$ e <br/>$s$ = {s}."
            request_lib = f"Fornire una massima sottosequenza comune tra il prefisso t' = " + str(tasks[i]['string_mod']) + "di t e s = "+ str(s)
            ...
            request=f"__Richiesta {i} [{tasks[i]['tot_points']} punti]__: Fornire una massima sottosequenza comune tra:<br/><b>il suffisso </b> $s'$ = {tasks[i]['string_mod']} di $s$ e <br/>$t$ = {t}."
            request_lib = f"Fornire una massima sottosequenza comune tra il suffisso s' = " + str(tasks[i]['string_mod']) + "di s e t = " + str(t)
            ...
            request=f"__Richiesta {i} [{tasks[i]['tot_points']} punti]__: Fornire una stringa di lunghezza massima che <b>inizi con la lettera </b> __{tasks[i]['start']}__ e sia sottosequenza comune tra: <br/>$s$ = {s}<br/>$t$ = {t}."
            request_lib = f"Fornire una stringa di lunghezza massima che inizi con la lettera " + str(tasks[i]['start']) + " e sia sottosequenza comune tra s = " + str(s) + " e t = " + str(t)
            ...
            request=f"__Richiesta {i} [{tasks[i]['tot_points']} punti]__: Fornire una stringa di lunghezza massima che <b>finisca con la lettera </b> __{tasks[i]['end']}__ e sia sottosequenza comune tra:<br/>$s$ = {s} <br/>$t$ = {t}."
            request_lib = f"Fornire una stringa di lunghezza massima che finisca con la lettera " + str(tasks[i]['end']) + " e sia sottosequenza comune tra s = " + str(s) + " e t = " + str(t)
        ...
        if tasks[i]['request'] != "R0":
            cell_type='Code'
            cell_string=f'#Inserisci la risposta (una stringa, per esempio "TATATA")\nanswer{num_of_question - 1}=""'
