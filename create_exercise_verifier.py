...
def create_exercise(exam_date, num, path_ex_folder, ex_type, path_yaml, path_almost_yaml):
    ...
    else: # ex_type == 'dp_robot_no_gemme' (TO DO: add other dynamic programming exercises here)
    ...
    exer = yaml_gen
    if 'tags' in exer:
        e_dict = {'title':exer['title'],'tags':exer['tags'],'tot_points':0,'link':'http://127.0.0.1:8888/notebooks/'+prev_folder+'/modo_verif/' + note_name, 'tasks':exer['tasks'], 'mode':'Verificatore'}
    else:
	    e_dict = {'title':exer['title'],'tags':[],'tot_points':0,'link':'http://127.0.0.1:8888/notebooks/'+prev_folder+'/modo_verif/' + note_name, 'tasks':exer['tasks'], 'mode':'Verificatore'}
